...
enum MovementFlags
{
    ...
    MOVEFLAG_MASK_MOVING =
        ...
        MOVEFLAG_PITCH_UP | MOVEFLAG_PITCH_DOWN | MOVEFLAG_JUMPING | MOVEFLAG_FALLING | MOVEFLAG_ASCENDING | MOVEFLAG_DESCENDING |
        MOVEFLAG_SPLINE_ELEVATION,
	MOVEFLAG_MASK_FLYING = 
	MOVEFLAG_AIR_SWIMMING | MOVEFLAG_CAN_FLY | MOVEFLAG_JUMPING | MOVEFLAG_FALLING | MOVEFLAG_FALLING_FAR | MOVEFLAG_FREE_FALLING | MOVEFLAG_NO_COLLISION,

	MOVEFLAG_MASK_CHECK_FLYING = 
	MOVEFLAG_AIR_SWIMMING | MOVEFLAG_CAN_FLY | MOVEFLAG_NO_COLLISION,
	MOVEFLAG_MASK_CHECK_JUMPING = 
	MOVEFLAG_FALLING | MOVEFLAG_FALLING_FAR | MOVEFLAG_FREE_FALLING,
...
class SERVER_DECL WorldSession
{
	...
	public:
       void SendLfgUpdatePlayer(const LfgUpdateData& updateData);
		...
		void SetAccountId(uint32 id) { _accountId = id; }
	private:
		friend class Player;
		Player* _player;
		WorldSocket* _socket;
