...
#ifdef WIN32
	Log.Success("Server", "Realm `%s` registered as realm %u.", _StringToANSI(realmname.c_str()), realmlid);
#else
	Log.Success("Server", "Realm `%s` registered as realm %u.", realmname.c_str(), realmlid);
#endif
