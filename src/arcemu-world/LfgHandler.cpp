...
void WorldSession::HandleLfgPlayerLockInfoRequestOpcode(WorldPacket& /*recv_data*/)
{
	...
	for (LfgDungeonSet::const_iterator it = randomDungeons.begin(); it != randomDungeons.end(); ++it)
	{
		...
		if (qRew)
		{
			...
			data << uint8(qRew->count_reward_choiceitem);
			if (qRew->count_reward_choiceitem)
			{
				ItemPrototype const* iProto = NULL;
				for (uint8 i = 0; i < 4; ++i)
				{
					if (!qRew->reward_choiceitem[i])
						continue;

					iProto = ItemPrototypeStorage.LookupEntry(qRew->reward_choiceitem[i]);

					data << uint32(qRew->reward_choiceitem[i]);
...
void WorldSession::SendLfgPlayerReward(uint32 rdungeonEntry, uint32 sdungeonEntry, uint8 done, const LfgReward* reward, const Quest* qRew)
{
	...
	uint8 itemNum = uint8(qRew ? qRew->count_reward_item : 0);
...
void WorldSession::SendLfgUpdateProposal(uint32 proposalId, const LfgProposal* pProp)
{
	...
	Group* grp = dLowGuid ? objmgr.GetGroupById(dLowGuid) : NULL;
	...
	if (LFGDungeonEntry const* dungeon = dbcLFGDungeonEntry.LookupEntry(dungeonId))
	{
