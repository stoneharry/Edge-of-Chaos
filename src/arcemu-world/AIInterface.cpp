...
void AIInterface::Init(Unit* un, AIType at, MovementType mt)
{
	...
	m_walkSpeed = m_Unit->m_walkSpeed * 0.01f; //move distance per ms time
	m_runSpeed = m_Unit->m_runSpeed * 0.01f; //move distance per ms time
	m_flySpeed = m_Unit->m_flySpeed * 0.01f;
...
void AIInterface::Update(uint32 p_time)
{
	...
	if(m_AIState == STATE_EVADE)
	{
		...
		if(tdist <= 4.0f)
		{
			...
			if(m_AIType != AITYPE_PET && !skip_reset_hp && !(m_Unit->IsCreature() && m_Unit->HasCreatureCustomFlag(CREATURE_CUSTOMFLAG_CANT_REGEN_HEALTH)))
...
void AIInterface::UpdateSpeeds()
{
	if(HasWalkMode(WALKMODE_SPRINT))
		m_runSpeed = (m_Unit->m_runSpeed + 5.0f) * 0.01f;
	if(HasWalkMode(WALKMODE_RUN))
		m_runSpeed = m_Unit->m_runSpeed * 0.01f;

	m_walkSpeed = m_Unit->m_walkSpeed * 0.01f;
	m_flySpeed = m_Unit->m_flySpeed * 0.01f;
	m_Unit->UpdateSpeed();
}
