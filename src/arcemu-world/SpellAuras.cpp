	...
	if(apply)
	{
		...
		if(caster != NULL)
		{
			if(Player * p = caster->GetSpellModOwner())
				p->ApplySpellMod(GetSpellProto()->Id, SPELLMOD_BONUS_MULTIPLIER, amptitude);
...
void Aura::SpellAuraForceReaction(bool apply)
{
	...
	WorldPacket data(SMSG_SET_FORCED_REACTIONS, 4+p_target->m_forcedReactions.size()*(4+4));
...
void AbsorbAura::SpellAuraSchoolAbsorb(bool apply)
{
	...
	if(caster != NULL)
	{
		...
		int flat = 0;
		if(Player * p = caster->GetSpellModOwner())
			p->ApplySpellMod(GetSpellProto()->Id, SPELLMOD_BONUS_MULTIPLIER, flat);
		val += val * flat / 100;
