...
{
	if(m_fallDisabledUntil < time(NULL) + 5) m_fallDisabledUntil = time(NULL) + 5; //VLack: If the ride wasn't free, the player shouldn't die after arrival because of fall damage... So we'll disable it for 5 seconds.
...
void Player::_EventExploration()
{
	...
	uint32 val = (uint32)(1 << (at->explorationFlag % 32));
	uint32 currFields = GetUInt32Value(offset);
	if(AreaId== m_zoneId || AreaId == m_AreaID)
		return;
	...
	if(proto->ScalingStatsEntry != 0)
	{
		...
		if(ssdrow && ssvrow)
		{
			for (uint8 i = 0; i < 10; ++i)
			{
				...
				if (ssdrow && ssvrow)
				{
					if (ssdrow->StatMod[i] < 0)
						continue;
					statType = ssdrow->StatMod[i];
					val = (ssvrow->getssdMultiplier(proto->ScalingStatsFlag) * ssdrow->Modifier[i]) / 10000;
					ModifyBonuses(statType, val, apply);
				}
			}
			if (int32 spellbonus = ssvrow->getSpellBonus(proto->ScalingStatsFlag))
				ModifyBonuses(45, spellbonus, apply);
			if (uint32 ssvarmor = ssvrow->getArmorMod(proto->ScalingStatsFlag))
			{
				if(apply)
			...
			if(int32 extraDPS = ssvrow->getDPSMod(proto->ScalingStatsFlag))
			{
...
int32 Player::CanShootRangedWeapon(uint32 spellid, Unit* target, bool autoshot)
{
	...
	ApplySpellMod(spellid, SPELLMOD_RANGE, maxr, NULL);
...
void Player::Cooldown_Add(SpellEntry* pSpell, Item* pItemCaster)
{
	...
	if(pSpell->CategoryRecoveryTime > 0 && pSpell->Category)
	{
		...
		if (cool_time > 0)
            ApplySpellMod(pSpell->Id, SPELLMOD_COOLDOWN, cool_time, NULL);
		if(cool_time < 0)
			cool_time = 0;
...
bool Player::IsAffectedBySpellmod(SpellEntry * spellInfo, SpellModifier* mod, Spell* spell)
{
    if (!mod || !spellInfo)
        return false;
...
{
	if(GetCurrentSpell())
		RestoreSpellMods(GetCurrentSpell(), ownerAuraId, aura);
}
...
{
    return IsInWater() &&
		GetPositionZ() < (GetMapMgr()->GetLiquidHeight(GetPositionX(), GetPositionY())-2);
}
    ...
    if (m_MirrorTimerFlags & UNDERWARER_INDARKWATER)
    {
        ...
        else
        {
            ...
            if (m_MirrorTimer[FATIGUE_TIMER] < 0)
            {
                ...
                {
                    uint32 damage = GetMaxHealth() / 5 + RandomUInt(getLevel()-1);
 					SendEnvironmentalDamageLog(GetGUID(), DAMAGE_EXHAUSTED, damage);
					DealDamage(this, damage, 0, 0, 0);
               }
...
int32 Player::getMaxTimer(MirrorTimerType timer)
{
    switch (timer)
    {
        ...
        case BREATH_TIMER:
        {
            if (!isAlive() || HasAuraWithName(SPELL_AURA_WATER_BREATHING))
