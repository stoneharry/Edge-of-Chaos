...
<xsl:template match="/">
	...
	<head>
		...
		@import url(server_stats.css);.style3 {font-size: 9px}
        </style>
		<script src="realminfo.js">/* Wee, urine */</script>
	</head>
	<body id="top">
		<center><div style="width: 98%;">
		  <h1>ArcEmu Server Status<br />
	      <span class="style3"><a href="http://www.arcemu.org/">http://www.arcemu.org</a></span></h1>
		  <div class="about_this_page">
		<b>About this page</b><br/>
		The <i>Server Status</i> page is automatically generated by the server and provides very basic information about what is happening ingame.<br/>
		You can see who is online, and what class, race, or level they are. You can even see <i>where</i> they are, to an extent.<br/>
		<b>Jump To:</b><br/>
		[<a href="#coreinfo">Core Info</a>]<br/>
		[<a href="#InstanceInfo">Active Instances</a>]<br/>
		[<a href="#GMInfo">GMs Online</a>]<br/>
		[<a href="#PlayerInfo">Players Online</a>]		</div>
		<br/><br/>
		<xsl:apply-templates/>
...
<xsl:template match="status">
	<table width="100%" border="0" cellspacing="0" cellpadding="3" class="stattable" id="coreinfo">
		<tr>
			<th colspan="4" class="player_online_count">Core Info</th>
		</tr>
		<tr>
			<th class="table_header" width="25%">Platform: </th>
			<td class="row lpad12" width="25%"><xsl:value-of select="platform"/></td>
			<th class="table_header" width="25%">Uptime: </th>
			<td class="row lpad12" width="25%"><xsl:value-of select="uptime"/></td>

		</tr>
		<tr>
			<th class="table_header alternate">Online Players: </th>
			<td class="row_alternate_row lpad12"><xsl:value-of select="oplayers"/> [<a href="#PlayerInfo">Show all players</a>]</td>
			<th class="table_header alternate">CPU Usage: </th>
			<td class="row_alternate_row lpad12"><xsl:value-of select="cpu"/>%</td>
		</tr>
		<tr>

			<th class="table_header">Queued Players: </th>
			<td class="row lpad12"><xsl:value-of select="qplayers"/></td>
			<th class="table_header">Memory Usage: </th>
			<td class="row lpad12"><xsl:value-of select="ram"/> MB</td>
		</tr>
		<tr>
			<th class="table_header alternate">Average Latency: </th>
			<td class="row_alternate_row lpad12"><xsl:value-of select="avglat"/> ms</td>
			<th class="table_header alternate">Thread Count: </th>
			<td class="row_alternate_row lpad12"><xsl:value-of select="threads"/></td>

		</tr>
		<tr>
			<th class="table_header">Online GM Count: </th>
			<td class="row lpad12"><xsl:value-of select="gmcount"/></td>
			<th class="table_header">Last Update: </th>
			<td class="row lpad12"><xsl:value-of select="lastupdate"/></td>
		</tr>
		<tr>
			<th class="table_header alternate">Alliance Online: </th>
			<td class="row_alternate_row lpad12"><xsl:value-of select="alliance"/></td>
			<th class="table_header alternate">Accepted Connections:</th>
			<td class="row_alternate_row lpad12"><xsl:value-of select="acceptedconns"/></td>
		</tr>
		<tr>
			<th class="table_header">Horde Online:</th>
			<td class="row lpad12"><xsl:value-of select="horde"/></td>
			<th class="table_header">Connection Peak:</th>
			<td class="row lpad12"><xsl:value-of select="peakcount"/></td>
		</tr>
		<tr>
			<th class="table_header alternate">Delayed Queries (World DB): </th>
			<td class="row_alternate_row lpad12"><xsl:value-of select="wdbquerysize"/></td>
			<th class="table_header alternate">Delayed Queries (Character DB): </th>
			<td class="row_alternate_row lpad12"><xsl:value-of select="cdbquerysize"/></td>
		</tr>
	</table>
</xsl:template>
...
<div id="InstanceInfo">Instance information loading...</div>
<script language="javascript">
<xsl:for-each select="instance">AddInstance( &quot;<xsl:choose>
...
<h2>GMs Online</h2>
<div id="GMInfo">Gamemaster info loading...</div>
<script language="javascript">
		<xsl:for-each select="gmplr">
		<xsl:sort select="level" data-type="number"/>AddGM( &quot;<xsl:value-of select="name"/>&quot; , &quot;<xsl:value-of select="race" />&quot; , &quot;<xsl:value-of select="class" />&quot; , <xsl:value-of select="level"/> , &quot;<xsl:value-of select="ontime"/>&quot; , &quot; <xsl:value-of select="latency"/>&quot; , &quot;<xsl:value-of select="permissions" />&quot; , &quot;<xsl:value-of select="gender" />&quot; );
		</xsl:for-each>
DoSort( 1 , 2 );
</script>
</xsl:template>
...
<div id="PlayerInfo">Loading players online...</div>

<script language="javascript">
<xsl:for-each select="plr">AddPlayer( '<xsl:value-of select="name" />' , <xsl:value-of select='level' /> , <xsl:value-of select='race' /> , <xsl:value-of select='class' /> , &quot;<xsl:choose>
...
<xsl:when test="map = 624">Vault of Archavon</xsl:when>
            ...
            </xsl:choose>&quot; , &quot;<xsl:value-of select="ontime"/>&quot; , <xsl:value-of select="gender" /> , <xsl:value-of select="latency" /> );
			</xsl:for-each>
/* Sort it and display */
DoSort( 1 , 0 ); /* Automatically sort by level */
/* No need to Display();, since DoSort does it automatically */
//Display();
</script>
